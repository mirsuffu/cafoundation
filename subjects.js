  ...
  data.subjects.forEach(function (subj) {
    ...
    subj.chapters.forEach(function (ch, ci) {
      ...
      statSel.addEventListener('change', () => { 
        ...
        if (typeof window.logActivity === 'function') window.logActivity('Updated Chapter Status', `${subj.name} - ${ch.name} - ${ch.status}`);
        saveData(); 
      ...
      (function (ch, flag) { makeLevelDropdown(ds, ch.difficulty, function (v) { ch.difficulty = v; if (typeof window.logActivity === 'function') window.logActivity('Updated Chapter Difficulty', `${subj.name} - ${ch.name}`); saveData(); flag.innerHTML = getPriorityFlag(ch.difficulty, ch.confidence); refreshSubjectMeta(subj, header); }); })(ch, flag);
      ...
      (function (ch, flag) { makeLevelDropdown(cs, ch.confidence, function (v) { ch.confidence = v; if (typeof window.logActivity === 'function') window.logActivity('Updated Chapter Confidence', `${subj.name} - ${ch.name}`); saveData(); flag.innerHTML = getPriorityFlag(ch.difficulty, ch.confidence); refreshSubjectMeta(subj, header); }); })(ch, flag);
      ...
      if (mobile) {
        ...
        if (editorUnlocked) {
          ...
          (function (ci) { del.addEventListener('click', function () { playSound('click'); var cName = subj.chapters[ci].name; subj.chapters.splice(ci, 1); if (typeof window.logActivity === 'function') window.logActivity('Deleted Chapter', `${subj.name} - ${cName}`); saveData(); renderSubjectsInternal(); }); })(ci);
    ...
    if (editorUnlocked) {
      ...
      (function (subj) { ab.addEventListener('click', function () { playSound('click'); openSubjects.add(subj.id); subj.chapters.push({ id: generateId('ch'), name: 'New Chapter', difficulty: 2, confidence: 2, status: 'Pending' }); if (typeof window.logActivity === 'function') window.logActivity('Added Chapter', subj.name); saveData(); renderSubjectsInternal(); }); })(subj);
