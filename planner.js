  ...
  dates.forEach(date => {
    ...
    if (mobile) {
      ...
      bulk.addEventListener('click', () => {
        const hasPlans = SUBJECTS.some(s => row.plans[s] && row.plans[s].trim() !== '');
        if (!hasPlans) {
      ...
      SUBJECTS.forEach(subj => {
        ...
        tick.addEventListener('click', () => {
          if (!row.plans[subj] || row.plans[subj].trim() === '') {
      ...
      abtn.addEventListener('click', () => {
        ...
        tr.querySelectorAll('.tick-btn').forEach((b, i) => {
          const s = SUBJECTS[i];
          b.className = 'tick-btn' + (row.ticks[s] ? ' ticked' : '');
          b.textContent = row.ticks[s] ? '✓' : '';
        });
        if (currentSection === 'metrics') renderMetrics();
      });
