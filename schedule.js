...
function renderSchedule() {
  SCHED_KEYS.forEach((key, i) => {
    ...
    if (slots.length === 0 && !editorUnlocked) { body.innerHTML = '<div class="empty-schedule">No slots added yet.</div>'; }
      ...
      slots.forEach((slot, si) => {
        ...
        if (editorUnlocked) {
          ...
          del.addEventListener('click', () => { data.schedules[key].slots.splice(si, 1); if (typeof window.logActivity === 'function') window.logActivity('Deleted Schedule Slot', key); saveData(); renderSchedule(); });
    ...
    if (editorUnlocked) {
      ...
      ab.addEventListener('click', () => { data.schedules[key].slots.push({ start: '', end: '', label: '' }); if (typeof window.logActivity === 'function') window.logActivity('Added Schedule Slot', key); saveData(); renderSchedule(); });
      body.appendChild(ab);
    }
  });
}
