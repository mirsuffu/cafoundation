...
async function preloadSounds() {
  try {
    const [clickRes, popRes] = await Promise.all([
      fetch('click.mp3'),
      fetch('pop.mp3')
    ]);
    if (clickRes.ok) sounds.click.src = URL.createObjectURL(await clickRes.blob());
    if (popRes.ok) sounds.pop.src = URL.createObjectURL(await popRes.blob());
  } catch (e) {
...
function toDateStr(d) { return toDateStrSimple(d); }
